...
function M:view(opts)
    return Promise.new(function(resolve, reject)
        local function do_view()
            ...
            local image = api.from_file(opts.path, { window = win, buffer = buf, x = 0, y = 0 })

            -- Set the image's width to be that of our window (in pixels) and
            -- scale height by same amount
            local scale = (width * term.get_size().cell_width) / image.image_width
            print("IMAGE PRE-SCALE", image.image_width, "x", image.image_height)
            print("SCALE", scale)
            image.image_width = image.image_width * scale
            image.image_height = image.image_height * scale
            print("IMAGE POST-SCALE", image.image_width, "x", image.image_height)
